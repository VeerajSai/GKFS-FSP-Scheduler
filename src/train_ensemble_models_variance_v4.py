"""
Variance-Based Ensemble Model Training - V4
==============================================

End-to-end ML pipeline for wind power prediction using variance-based splitting.
Includes SVM models and stacking ensembles for high variance periods.

Key Features:
- Variance-based splitting (LOW_VARIANCE vs HIGH_VARIANCE)
- Enhanced feature engineering with variance-specific features
- SVM models for both variance types
- Stacking ensemble for high variance (without tree models to avoid overfitting)
- Weighted ensemble for low variance
- No Optuna tuning (uses proven default parameters)
"""
...
def get_low_variance_models(random_state: int = 42) -> Dict[str, BaseEstimator]:
...
def get_high_variance_models(random_state: int = 42) -> Dict[str, BaseEstimator]:
    ...
    # SVM (RBF kernel - for complex patterns)
    models['svm_rbf'] = SVR(
        kernel='rbf',
        C=100.0,
        gamma='scale',
    ...
    # SVM (Polynomial kernel - for non-linear patterns)
    models['svm_poly'] = SVR(
        kernel='poly',
        degree=2,
        C=10.0,
...
def get_stacking_ensemble_models(random_state: int = 42) -> Dict[str, BaseEstimator]:
    ...
    base_models = [
        ...
        ('svm_rbf', SVR(kernel='rbf', C=10.0, gamma='scale', epsilon=0.1)),
...
class VarianceModelTrainer:
    ...
    def train_models(
        ...
        if var_type == 'low_variance':
            ...
            base_models = get_low_variance_models(self.config.random_state)
            ...
            base_models = get_high_variance_models(self.config.random_state)
    ...
    def run(self):
        ...
        _print_section("VARIANCE-BASED MODEL TRAINING - V4")
